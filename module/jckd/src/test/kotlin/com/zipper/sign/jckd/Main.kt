...
fun main() = runBlocking(newSingleThreadContext("zqkd")) {
    val configJson = Thread.currentThread().contextClassLoader
        .getResourceAsStream("jckd_config.json")?.use {
        val reader = it.reader(StandardCharsets.UTF_8)
        reader.readText()
    }

    val list: List<JckdApiParam> = Gson().fromJson(configJson, object : TypeToken<List<JckdApiParam>>() {}.type)
    list.map {
        async { JckdApi().execute(it) }
    }.forEach {
        it.await()
    }
