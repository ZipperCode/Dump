package com.zipper.sign.kugou

import com.zipper.sign.kugou.bean.*
import okhttp3.MultipartBody
import okhttp3.RequestBody
import retrofit2.Response
import retrofit2.http.*

interface KgTaskService {

    @Headers(
        "KG-RC: 1",
        "KG-RF: 8002dddd",
        "KG-THash: 439de79",
    )
    @GET("musicsymbol/v1/system/profile")
    suspend fun profile(@QueryMap params:Map<String, String>): KgTaskProfile

    /**
     * userid=1882565136&token=&appid=1005&from=client&dfid=-&signature=a19de0ce9a8f25d42b7b589c7bf2130f&mid=19866310614991365975980225640804152227&clientver=10829&clienttime=1630849668&uuid=532544371bb026cc684489e57eedbe9c&
     */
    @Headers(
        "KG-RC: 1",
        "KG-RF: 80023fff",
        "KG-THash: 697fce7",
    )
    @GET("musicsymbol/v1/task/sign_state")
    suspend fun signState(@QueryMap params:Map<String, String>): KgTaskSignState

    /**
     * ?srcappid=2919&clientver=10829&clienttime=1630849669476&mid=19866310614991365975980225640804152227&uuid=532544371bb026cc684489e57eedbe9c&dfid=-&userid=1882565136&token=&appid=1005&from=client&spec=15&channel=musicsymbol10829&signature=0fb472334979c31729f26a9b2ffbb6dc
     */
    @Headers(
        "KG-RC: 1",
        "KG-RF: 8002d918",
        "KG-THash: 697fce7",
    )
    @POST("musicsymbol/v1/task/state_list")
    suspend fun stateList(@QueryMap params: Map<String, String>, @Body body: RequestBody): KgTaskStateList

    /**
     * userid=1882565136&token=&appid=1005&from=client&spec=15&dfid=-&signature=f7ccffd6a1b6192a5bed8a9eb61e6704&mid=19866310614991365975980225640804152227&clientver=10829&clienttime=1630849663&uuid=532544371bb026cc684489e57eedbe9c&
     */
    @Headers(
        "KG-RC: 1",
        "KG-RF: 8002d102",
        "KG-THash: 697fce7",
    )
    @GET("musicsymbol/v1/user/info")
    suspend fun info(@QueryMap params: Map<String, String>): KgTaskInfo

    /**
     * userid=1882565136&token=&appid=1005&from=client&dfid=-&mid=19866310614991365975980225640804152227&clientver=10829&clienttime=1630849673&uuid=532544371bb026cc684489e57eedbe9c&signature=9bc7a3a990b805411cbc2ee3bf4ad814&
     */
    @Headers(
        "KG-RC: 1",
        "KG-RF: 800247d9",
        "KG-THash: 697fce7",
    )
    @GET("musicsymbol/v1/gift/list")
    suspend fun list(@QueryMap params: Map<String, String>): Response<KgTaskList>

    @Headers(
        "KG-RC: 1",
        "KG-RF: 8002b24f",
        "KG-THash: 22fd1ac",
    )
    @POST("musicsymbol/v1/task/submit")
    suspend fun submit(@QueryMap params: Map<String, String>, @Body body: RequestBody): KgTaskSubmit

    /**
     * 酷狗任务签到
     * param {"code":"20210905"}
     */
    @Headers(
        "KG-RC: 1",
        "KG-RF: 0075da00",
        "KG-THash: 697fce7",
        "Content-Type: application/x-www-form-urlencoded"
    )
    @POST("musicsymbol/v1/task/signon")
    suspend fun signOn(
        @QueryMap params:Map<String, String>,
        @Body requestBody: RequestBody
    ): KgTaskSignon
}